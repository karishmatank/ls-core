...
-- See advanced_2_erd.png

/*
We need a list of the organizations that have an active subscription.
*/
...
/*
We need a list of all of the organizations and the plan that they're currently on.

I used left joins in case there are organizations that don't have a subscription listed in the subscriptions table or a plan.
*/
...
/*
How many organizations are on an active paid plan (any plan that costs more than 0)?
*/
SELECT count(o.id)
FROM organizations o
JOIN subscriptions s on s.organization_id = o.id
JOIN plans p ON s.plan_id = p.id
WHERE p.monthly_price > 0 AND s.status = 'active';


/*
Show the total monthly revenue from all active subscriptions.
*/
...
-- NOTE from solution:  In this solution, we've decided to "trust" the subscriptions table. A more robust solution would join with the organizations table
-- In this case, we get the same results. Our data is quite neat. However, it’s a good opportunity to recognize how important constraints and cascading deletions are. With messier data, you need to be more diligent in structuring queries to ensure you’re not misled by invalid or orphaned records.

/*
Show each organization and how many users it has. Put organizations with more users first.
*/

SELECT o.name, count(u.id)
FROM organizations o
...
/*
Show all users who belong to organizations that are on a paid plan. We want their name, email, and the organization they belong to.
*/

SELECT u.name, u.email, o.name
FROM users u
...
/*
We'd like to know, on average, how many users belong to each organization, regardless of their subscription status.
*/
SELECT round(avg(users_count), 2)
FROM (
  SELECT o.id, count(u.id) AS users_count
  FROM organizations o
  LEFT JOIN users u ON o.id = u.organization_id
  GROUP BY o.id
) AS org_users_count;
...
/*
We’d like to see the number of users on each type of plan, free, team, and pro.
*/
...
/*
We would like to run a campaign for organizations who have maxed out their free plan. What organizations are using their maximum alotted free plan seats?
*/

-- Essentially, which organizations are on a free plan and have user count that matches the max number of seats for the free plan?
...
WHERE p.name = 'Free' AND s.status = 'active'
GROUP BY o.name
HAVING count(u.id) = (SELECT max_seats FROM plans WHERE name = 'Free');

/*
The team plan currently allows 15 users per organization. We’re considering reducing it to 10 users. Which organizations would be affected? We need the organization name, number of users, and current plan.
*/
