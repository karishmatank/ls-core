...
SELECT *
FROM people
LIMIT 1 OFFSET 1; -- Assumes order of rows is same as above example
...
SELECT avg(wingspan) as round, min(wingspan) as min, max(wingspan) as max
FROM birds;
...
/* Using the table and data from #8 and #9, write a SQL query to determine which menu item is the most profitable based 
on the cost of its ingredients, returning the name of the item and its profit. */
...
List the most profitable items first. Keep in mind that prep_time is represented in minutes and 
...
SELECT 
  ...
  menu_price, 
  ingredient_cost, 
  prep_time * (13.0 / 60.0) AS labor, -- Be careful not to do integer division!
  menu_price - ingredient_cost - (prep_time * (13.0 / 60.0)) AS profit -- Can't access labor in this line, need to recalculate
FROM menu_items
ORDER BY profit DESC;