...
Each element in the DOM is represented as ["PARENT_TAG_NAME", [children]] where children are elements as well
and as such follow the same format. When an element has no children, it's represented as ["PARENT_TAG_NAME", []].
...
function getNodeArray(node) {
  ...
  children.forEach(child => {
    let childTree = getNodeArray(child);
    tree.push([child.tagName, childTree]);
  });
  return tree;
}

function nodesToArr() {
  return ["BODY", getNodeArray(document.body)];
}
