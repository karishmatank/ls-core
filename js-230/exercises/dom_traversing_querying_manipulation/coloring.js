...
Example usage:
https://d1nrfq3cstnmkv.cloudfront.net/exercises/dom_querying_manipulation/coloring.png
...
function colorGeneration(generation) {
  ...
  let currentLayer = [document.body];
  let nextLayer = [];
  for (let layer = 0; layer < generation; layer += 1) {
    currentLayer.forEach(element => {
      nextLayer = nextLayer.concat(Array.from(element.children));
    });
    currentLayer = nextLayer.slice();
    nextLayer = [];
  }

  currentLayer.forEach(element => element.classList.add('generation-color'));
}
