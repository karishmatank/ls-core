...
Assume that the grandest parent is the element with an id of "1". Use the following HTML and test cases to test your code:
...
function domTreeTracer(nodeId) {
  ...
  while (nodeId !== null && nodeId >= 1) {
    let parent = document.getElementById(nodeId).parentNode;
    let childrenTags = Array.from(parent.children).map(child => child.tagName);
    tree.push(childrenTags);
    nodeId = parent.id;
  }
