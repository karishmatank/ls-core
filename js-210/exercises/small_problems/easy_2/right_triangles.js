...
The hypotenuse of the triangle (the diagonal side in the images below) should have one end at the lower-left of
...
function triangle(numRows) {
  for (let row = 1; row <= numRows; row += 1) {
    console.log(`${' '.repeat(numRows - row)}${'*'.repeat(row)}`);
  }
}
