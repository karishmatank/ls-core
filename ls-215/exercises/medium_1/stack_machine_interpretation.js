...
My questions:
- Will the number provided always be positive integers? Might they be negative or floats?
- What happens if we receive an empty string?
- If a number is provided first, does it always initialize the register? Can a standalone `n` ever be provided in the middle of a string?
- Will the instructions always be delimited by single spaces?
- What happens if we come across an instruction that isn't valid?
- Will the input string always be in all uppercase?
- Will we always receive exactly 1 input? What if that input is not a string?
-


P:
Input: String
Output: Print to console if we receive a PRINT command
Rules:
- We'll have a stack, which is a group of values that grows or shrinks dynamically, initialized to []
- We'll also have a register, initialized to 0
- We remove number from the stack LIFO-style -> first number in is first number out
- Numbers in instructions can be negative. Assume they won't be floats though- they will always be integers
- If we receive an empty string, just return
- We only print to the console if we see PRINT
- Assume that our program won't try to access non-existent stack values
- Assume that the input will only contain existing "tokens"
...
function minilang(command) {
  ...
  steps.forEach(step => {
    ...
    } else if (!Number.isNaN(Number(step))) {
      register = Number(step);
    } else {
      console.log(`Token ${step} is invalid. Skipping...`);
    }
  });
